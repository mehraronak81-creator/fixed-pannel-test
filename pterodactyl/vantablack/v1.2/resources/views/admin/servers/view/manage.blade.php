...
@section('content')
    ...
    <section class="vh-suspension-summary {{ $server->isSuspended() ? 'is-suspended' : 'is-active' }}" aria-labelledby="vh-suspension-title">
        <div class="vh-suspension-icon" aria-hidden="true">
            <i class="fa {{ $server->isSuspended() ? 'fa-pause-circle' : 'fa-check-circle' }}"></i>
        </div>
        <div class="vh-suspension-copy">
            <span class="vh-eyebrow">Access control</span>
            <h2 id="vh-suspension-title">{{ $server->isSuspended() ? 'Server suspended' : 'Server access active' }}</h2>
            <p>{{ $server->isSuspended() ? 'User access, file management, API actions, and server processes are currently blocked.' : 'The owner can access and manage this server normally. Use suspension when access must be stopped immediately.' }}</p>
        </div>
        <a class="btn {{ $server->isSuspended() ? 'btn-success' : 'btn-warning' }}" href="#suspension-control">
            {{ $server->isSuspended() ? 'Review Unsuspend Action' : 'Review Suspend Action' }}
        </a>
    </section>
    <div class="row equal-height vh-server-actions">
        <div class="col-sm-4">
        ...
        @if(! $server->isSuspended())
            <div class="col-sm-4">
                <div id="suspension-control" class="box box-warning vh-suspension-card">
                    <div class="box-header with-border">
                    ...
                    <div class="box-footer">
                        <form action="{{ route('admin.servers.view.manage.suspension', $server->id) }}" method="POST">
                            ...
                            <button type="submit" class="btn btn-warning btn-block" @if(! is_null($server->transfer)) disabled aria-disabled="true" title="A server cannot be suspended during transfer." @endif>Suspend Access Now</button>
                        </form>
                    </div>
                </div>
            </div>
        @else
            <div class="col-sm-4">
                <div id="suspension-control" class="box box-success vh-suspension-card">
                    <div class="box-header with-border">
                    ...
                    <div class="box-footer">
                        <form action="{{ route('admin.servers.view.manage.suspension', $server->id) }}" method="POST">
                            ...
                            <button type="submit" class="btn btn-success btn-block">Restore User Access</button>
                        </form>
                    </div>
                </div>
            </div>
